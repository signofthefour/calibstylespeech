...
class   CalibratedStyleSpeechLoss(nn.Module):
    ...
    def __init__(self, preprocess_config, model_config):
        super(CalibratedStyleSpeechLoss, self).__init__()
        self.pitch_feature_level = preprocess_config["preprocessing"]["pitch"][
            "feature"
        ]
        self.energy_feature_level = preprocess_config["preprocessing"]["energy"][
    ...
    def forward(self, inputs, predictions):
        ...
        phoneme_content = torch.mean(phoneme_content, dim=1, keepdim=True)
        phoneme_content, mel_content = torch.flatten(phoneme_content), torch.flatten(mel_content)
